...
所有敏感操作都记录到 D1 审计日志（audit_logs 表），不可篡改。
...
**保留策略**：审计日志持久化存储在 D1 中，可通过管理面板查看和导出。当前未设置自动过期，建议定期归档清理旧日志。
...
### ADR-002: D1 主存储 + KV 有限缓存

- **决策**: D1 作为唯一主存储，KV 仅用于速率限制、通知节流和 GitHub API 监控
- **理由**:
  - D1 提供强一致性事务，避免 KV 最终一致性问题
  - 移除 KV 数据存储后，系统复杂度大幅降低
  - 仅保留 KV 的不可变用途（限流计数器、节流标记、监控指标）
  - 文件元数据、仓库配置、认证令牌、审计日志全部集中到 D1
- **影响**: 需要确保 D1 的查询性能满足请求时延要求
