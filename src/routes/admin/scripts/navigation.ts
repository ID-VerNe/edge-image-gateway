export const NAVIGATION = `
  ...
  async function init() {
    ...
    loadProviders(); // load providers silently in background
  }
  ...
  // Safe decode for URI-encoded path segments (handles already-decoded strings safely)
  ...
  async function loadRepos(data = null) {
    try {
      ...
      if(list) list.innerHTML = repos.map(repo => {
        ...
        const encodedRepoJson = encodeURIComponent(JSON.stringify(repo));
        ...
        return \`
          <div class="stat-card" style="position:relative; \${isCurrentWrite ? 'border-color: var(--primary);' : ''}">
            ...
            <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:1.25rem;">
              <div>
                <div style="font-size:1.125rem; font-weight:700; letter-spacing:-0.01em; margin-bottom:0.25rem;">\${eHtml(repo.id)}</div>
                <div style="font-size:0.8125rem; color:var(--text-2);">\${eHtml(repo.owner)}/\${eHtml(repo.name)} • \${eHtml(repo.branch)}</div>
              </div>
              <div style="display:flex; gap:0.5rem;">
                <button class="btn btn-mini" onclick="syncRepo('\${encodedRepoId}')">Sync</button>
                \${!isCurrentWrite ? \`<button class="btn btn-mini" onclick="setWriteRepo('\${encodedRepoId}')">Set Active</button>\` : ''}
                \${!isFallback ? \`
                  <button class="btn btn-mini" onclick='showEditRepoModal("\${encodedRepoJson}")'>Edit</button>
                  <button class="btn btn-mini btn-danger" onclick="deleteRepo('\${encodedRepoId}')">Delete</button>
                \` : ''}
              </div>
            </div>
  ...
  function renderSidebarTree(currentFiles) {
    ...
    pathParts.forEach((p, i) => {
      build += (i === 0 ? '' : '/') + p;
      html += \`
        <div class="tree-item active" style="padding-left: \${((i+1) * 1.5)}rem" onclick="loadFiles('\${encodeURIComponent(build)}')">
    ...
    folders.forEach(f => {
      html += \`
        <div class="tree-item" style="padding-left: \${((pathParts.length + 1) * 1.5)}rem" onclick="loadFiles('\${encodeURIComponent(f.path)}')">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
          \${eHtml(f.name)}
        </div>
      \`;
  ...
  function renderBreadcrumbs() {
    ...
    parts.forEach((p, i) => {
      ...
      html += '<span class="breadcrumb-item ' + cls + '" onclick="loadFiles(\\'' + encodeURIComponent(build) + '\\')">' + eHtml(p) + '</span>';
    });
    bc.innerHTML = html;
  }
  ...
  function switchView(v) {
    ...
    if(v === 'repos') {
      ...
      const providersNav = document.getElementById('nav-providers');
      if(providersNav) providersNav.classList.add('active');
      ...
      list.innerHTML = logs.map(log => {
        ...
        for (const k in log) {
          if (!['ts', 'user', 'action', 'ip'].includes(k)) {
            ...
            details += \`<div style="display:inline-block; margin-right:0.75rem;"><span style="color:var(--text-2); font-size:0.7rem; font-weight:600;">\${eHtml(k.toUpperCase())}</span> <span style="font-weight:500;">\${eHtml(String(val))}</span></div>\`;
          }
        }
        ...
        return \`
          <tr>
            ...
            <td style="font-weight:600; color:var(--primary);">\${eHtml(log.user)}</td>
            <td><span class="badge \${actionBadge}">\${eHtml(log.action.toUpperCase())}</span></td>
            <td style="font-family:monospace; font-size:0.75rem; color:var(--text-2);">\${eHtml(log.ip)}</td>
            <td>\${details}</td>
          </tr>
        \`;
      }).join('');
