  ...
  function renderFileList(files) {
    ...
    if (viewMode === 'list') {
      ...
      filteredFiles.forEach(f => {
        ...
        const epUrl = eUrl(f.path);
        html += \`
          <div class="file-row">
            <div><input type="checkbox" class="file-checkbox" onchange="toggleSelection('\${ep}')" \${selectedFiles.has(f.path) ? 'checked' : ''}></div>
            <div style="font-size: 1.25rem; display: flex; align-items: center; justify-content: center;">\${isDir ? '📁' : '🖼️'}</div>
            <div class="truncate"><a class="file-name" onclick="\${isDir ? \`loadFiles('\${ep}')\` : \`openLightbox('\${ep}', '/\${epUrl}')\`}">\${eHtml(f.name)}</a></div>
            <div style="color: var(--text-2);">\${isDir ? '-' : formatBytes(f.size)}</div>
            <div style="color: var(--text-2);">\${eHtml(f.type)}</div>
          </div>
        \`;
      });
      ...
      container.innerHTML = filteredFiles.map(f => {
        ...
        return \`
          <div class="grid-item">
            <input type="checkbox" class="file-checkbox" style="position:absolute; top:0.75rem; left:0.75rem; z-index:10;" onchange="toggleSelection('\${ep}')" \${selectedFiles.has(f.path) ? 'checked' : ''}>
            \${isDir
              ? \`<div class="grid-preview" onclick="loadFiles('\${ep}')">📁</div>\`
              : \`<img class="grid-preview" src="/\${epUrl}?w=300&q=70" onclick="openLightbox('\${ep}', '/\${epUrl}')" loading="lazy">\`
            }
            <div class="grid-info">
              <div class="grid-name" title="\${eHtml(f.name)}">\${eHtml(f.name)}</div>
              <div class="grid-actions">
                \${isDir
                  ? \`<button class="btn btn-mini" style="flex:1" onclick="loadFiles('\${ep}')">Open</button>\`
                  : \`
                    <button class="btn btn-mini btn-secondary" style="flex:1" onclick="copyLink('\${ep}')">Copy</button>
                    <button class="btn btn-mini" style="flex:1" onclick="showShareModal('\${ep}')">Share</button>
                    \`
                }
                <button class="btn btn-mini btn-danger" onclick="deleteItem('\${ep}', '\${isDir ? 'dir' : 'file'}')">
