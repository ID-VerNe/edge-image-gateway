      ...
      filteredFiles.forEach(f => {
        ...
        html += \`
          <div class="file-row" style="grid-template-columns: 20px 32px 3fr 1fr 1fr 160px;">
            ...
            <div><a class="file-name \${!isDir ? 'file' : ''}" onclick="\${isDir ? \`loadFiles('\${f.path}')\` : \`openLightbox('\${f.path}', '/\${f.path}')\`}">\${f.name}</a></div>
            <div class="file-meta">\${isDir ? '-' : (f.size/1024).toFixed(1) + 'KB'}</div>
            <div class="file-meta">\${f.type}</div>
            <div class="grid-actions">
              <button class="btn btn-mini" onclick="\${isDir ? \`loadFiles('\${f.path}')\` : \`openLightbox('\${f.path}', '/\${f.path}')\`}">\${isDir ? 'Open' : 'View'}</button>
        ...
        return \`
          <div class="grid-item \${isDir ? 'folder-grid-item' : ''}" style="position:relative;">
            ...
            \${isDir 
              ...
              : \`<img class="grid-preview" src="/\${f.path}?w=300&q=70" onclick="openLightbox('\${f.path}', '/\${f.path}')">\`
            }
            <div class="grid-info">
