...
| **存储** | 多仓库支持 | 支持多 GitHub 仓库，按容量自动路由写入，按路径规则路由读取 |
...
| **可观测性** | 监控告警 | 结构化日志、Sentry 错误追踪、Telegram 实时告警通知 |
| **可观测性** | 审计日志 | 记录所有上传、删除、配置变更等敏感操作，支持追溯 |
| **自动化** | 定时任务 | 通过 Cron Trigger 自动同步仓库容量统计 |
| **自动化** | 路径索引 | 上传后自动回填 KV 路径索引，加速后续请求路由 |
